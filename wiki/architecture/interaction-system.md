...
Current-run natural-language requirements are preserved by the `Explicit User Constraint Contract`. `ExplicitConstraintExtractor` records structured fields with `source=human_explicit`, evidence, confidence, and `priority=HARD`; QUICK and AI_DECIDE may resolve only unknown fields, while USER_DECIDE rejects contradictory updates with `CURRENT_USER_CONSTRAINT_CONFLICT`. `VisualSpecificationContract`, `PromptAdherenceManifest`, and `PromptCompiler` carry the same records in a dedicated `USER EXPLICIT HARD REQUIREMENTS` block. `ExplicitConstraintCoverageGate` runs after prompt compilation and blocks `GENERATION_READY` on dropped or conflicted human hard fields; unstructured hard requests remain as `NOT_REPRESENTABLE` raw constraints.
