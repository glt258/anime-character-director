...
The resolver consumes only packaged reviewed profiles under `references/game_styles/`. v2 profiles expose a structured rendering signature: contour, internal edge, primary shadow, secondary gradient, detail frequency, and explicitly marked `no reliable discriminator` slots. `StylePromptProjector` emits 5–8 core HOW rules plus at most 2 supporting rules, each with evidence, confidence, strength, and a contrastive HOW instruction. PromptCompiler places them in a `Rendering specialization:` section; the game name remains metadata/debug only. Explicit user rendering preferences are processed first and each rule records `applied`, `adapted`, or `dropped` with a conflict reason. An unsupported game keeps the original request and fallback reason for audit, then proceeds with the global style without mapping to another game. The `STYLE_DIFFERENCE_VALIDITY_GATE` requires both character preservation PASS and a clear character-subject rendering difference; a background-only or `SUBTLE_DIFFERENCE` result is not sufficient. The global Gacha Style Gate, pose rules, anatomy gate, and literal-copy protection remain mandatory.
...
`AdherenceDispositionPolicy` is a separate decision layer after the Critic. The Critic's strict `overall_result` remains the truth about visual fidelity; the policy adds independent ownership (`HUMAN_EXPLICIT`, `HUMAN_SELECTION`, `SYSTEM_INVARIANT`, `AI_RESOLVED`, `SYSTEM_DEFAULT`), `repair_importance`, `repair_risk`, and a replay-stable `AdherenceDisposition` / `RepairTriggerDecision`. AI-owned or system-default deviations remain visible, but do not automatically trigger full-image repair when human-owned constraints and system invariants pass. Only policy-approved `actionable_repair_targets` enter `VisualRepairPlan`; a user-requested manual repair may explicitly promote an informational target.

## TARGETED VISUAL REPAIR LOOP

An explicitly authorized repair uses only the current run's policy-approved `actionable_repair_targets`, current `PromptBundle`, `VisualSpecificationContract`, and `PromptAdherenceManifest`. It must not read previous runs or rerun Character Direction, Art Direction, DesignDNA generation, QUICK/AI_DECIDE/USER_DECIDE, or any diversity strategy. `PASS` HARD fields become structured `locked_fields` and are preserved exactly; only actionable targets enter `TARGETED REPAIR`. `TYPE 3` uses replacement instructions, while `TYPE 4` strengthens the named feature without redesigning the rest.
