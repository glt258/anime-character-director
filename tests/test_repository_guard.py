...
def test_installed_skill_path_detection() -> None:
    assert is_installed_skill_path(Path("example-user/.codex/skills/anime-character-director"))
    assert not is_installed_skill_path(Path("development/anime-character-director-release"))


def test_guard_rejects_installed_copy(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "scripts.repository_guard.repository_root",
        lambda _start=None: Path("example-user/.codex/skills/anime-character-director"),
    )
    with pytest.raises(DevelopmentLocationError, match="Installed Skill directory"):
        require_development_repository(Path("example-user/.codex/skills/anime-character-director"))
